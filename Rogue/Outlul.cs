...
{
    public class Outlul : CombatRoutine
    {
        #region Startup
        ...
        {
            get { return "Outlul, Outlaw Rogue rotation - version:" + _versionnumber; }
        }
        ...
        private static bool _zandebugger;
        private static string _zandebuggerstring = "None";
        private static bool _isOpenerDone;
        private static readonly string _versionnumber = "4.0.2 (7.3.0) Return to CS";
        private OutlulInterface Interface { get; set; }
        ...
        {
            WriteLog("Outlul by zan - v" + _versionnumber);
        ...
        {
            Log.Write("Zan: " + message, Color.SaddleBrown);
        }
        ...
        private static bool HasItem(Item item)
        {
            switch (item)
            {
                case Item.Shivarran: return WoW.IsItemEquipped("Shivarran Symmetry");                
        ...
        private static bool HasTalent(Talent talent)
        {
            switch (talent)
            {
                case Talent.GhostlyStrike: return WoW.IsTalentAvailable("GhostlyStrike");
                case Talent.BladeRush: return WoW.IsTalentAvailable("BladeRush");
                case Talent.QuickDraw: return WoW.IsTalentAvailable("QuickDraw");               
                case Talent.DeeperStratagem: return WoW.IsTalentAvailable("DeeperStratagem");                
                case Talent.KillingSpree: return WoW.IsTalentAvailable("KillingSpree");
                case Talent.SliceandDice: return WoW.IsTalentAvailable("SliceandDice");
                case Talent.MarkedforDeath: return WoW.IsTalentAvailable("MarkedforDeath");
        ...
        private static bool HasAura(Auras aura)
        {
            switch (aura)
            {
                case Auras.Stealth: return WoW.PlayerHasBuff("Stealth");
                case Auras.SkullandCrossbones: return WoW.PlayerHasBuff("SkullandCrossbones");
                case Auras.SliceandDice: return WoW.PlayerHasBuff("SliceandDice");
                case Auras.GrandMelee: return WoW.PlayerHasBuff("GrandMelee");
                case Auras.Vanish: return WoW.PlayerHasBuff("Vanish");
                case Auras.BuriedTreasure: return WoW.PlayerHasBuff("BuriedTreasure");
                ...
                case Auras.Mantle: return WoW.PlayerHasBuff("Mantle");
                case Auras.GhostlyStrike: return WoW.TargetHasDebuff("Ghostly Strike");
                case Auras.CotB: return WoW.TargetHasDebuff("Curse of the Dreadblades");
                case Auras.AdrenalineRush: return WoW.PlayerHasBuff("AdrenalineRush");
                default: return false;
            }
        }
        ...
        private static int AuraRemaining(Auras aura)
        {
            switch (aura)
            {
                case Auras.SkullandCrossbones: return WoW.PlayerBuffTimeRemaining("SkullandCrossbones");
                case Auras.SliceandDice: return WoW.PlayerBuffTimeRemaining("Slice and Dice");
                case Auras.GrandMelee: return WoW.PlayerBuffTimeRemaining("GrandMelee");
                case Auras.BuriedTreasure: return WoW.PlayerBuffTimeRemaining("BuriedTreasure");
        ...
        private static int SpellCD(Spells spell)
        {
            switch (spell)
            {
                case Spells.AdrenalineRush: return WoW.PlayerCooldownTimeRemaining("Adrenaline Rush");
                case Spells.CotDB: return WoW.PlayerCooldownTimeRemaining("Curse of the Dreadblades");
                case Spells.GhostlyStrike: return WoW.PlayerCooldownTimeRemaining("GhostlyStrike");
                case Spells.MarkedforDeath: return WoW.PlayerCooldownTimeRemaining("MarkedforDeath");
                default: return 0;
            }
        }
        ...
        private static bool AutoAoECheck
        {
            ...
            set { ConfigFile.WriteValue("Outlul", "AutoAoECheck", value.ToString()); }
        }
        ...
        {
            get { return ConfigFile.ReadValue<bool>("Outlul", "AutoAoECheck"); }
        }
        ...
        private static bool PreCheck(Precheck ability)
        {
            switch (ability)
            {
                case Precheck.Combat: return (WoW.PlayerHasTarget || WoW.TargetIsBoss) && (WoW.PlayerIsInCombat || HasAura(Auras.Vanish)) && WoW.TargetIsEnemy && WoW.IsSpellInRange("SinisterStrike") && WoW.PlayerHealthPercent > 0;
                case Precheck.Stealth: return (HasAura(Auras.Stealth) || HasAura(Auras.Vanish));              
                default:
                    return false;
            }
        }
        ...
        private static int RtbCount
        {
            get
            {
                return (Convert.ToInt32(HasAura(Auras.Broadside)) +
                        Convert.ToInt32(HasAura(Auras.BuriedTreasure)) +
                        Convert.ToInt32(HasAura(Auras.GrandMelee)) +
                        Convert.ToInt32(HasAura(Auras.RuthlessPrecision)) +
                        Convert.ToInt32(HasAura(Auras.SkullandCrossbones)) +
                        Convert.ToInt32(HasAura(Auras.TrueBearing)));
            }
        }



        private static int RTBRemaining
        {
            get
            {
                return Math.Max(AuraRemaining(Auras.SkullandCrossbones), Math.Max(AuraRemaining(Auras.GrandMelee),
                    Math.Max(AuraRemaining(Auras.RuthlessPrecision), Math.Max(AuraRemaining(Auras.TrueBearing),
                        Math.Max(AuraRemaining(Auras.BuriedTreasure), AuraRemaining(Auras.Broadside))))));
            }
        }
        ...
        //ame=rtb_reroll,value=rtb_buffs<2&(buff.loaded_dice.up|!buff.grand_melee.up&!buff.ruthless_precision.up)
        private static bool Rtb_reroll
        {
            get
            {
                return (RtbCount < 2 && (!HasAura(Auras.GrandMelee) && !HasAura(Auras.RuthlessPrecision)));
            }
        }
        ...
        private static bool Ambush_condition
        {
            get
            {
                return (Convert.ToBoolean(ComboPointsDeficit >= 2 + 2 * (Convert.ToInt32(HasTalent(Talent.GhostlyStrike) && SpellCD(Spells.GhostlyStrike) < 1)) + Convert.ToInt32(HasAura(Auras.Broadside)) && Power(Unit.Energy) > 60 & !HasAura(Auras.SkullandCrossbones)));
            }
        }

        private static readonly Stopwatch CombatTimer = new Stopwatch();
        private static Stopwatch BFTimer = new Stopwatch();
        //private static TimeSpan CombatTS = CombatTimer.Elapsed;
        private static double BFSec
        {
            get { return BFTimer.Elapsed.TotalSeconds; }
        }
        ...
        public void stealth()
        {
            if (SpellCast("Ambush", Ambush_condition)) return;
        }
        #endregion
        ...
        public void finish()
        {
            ...
            if (SpellCast("RolltheBones", (RTBRemaining <= 3 || Rtb_reroll))) return;
            ...
            if (SpellCast("BetweentheEyes", HasAura(Auras.RuthlessPrecision))) return;
            //-- actions.finish +=/ dispatch
            if (SpellCast("Dispatch")) return;
        }
        #endregion
        ...
        public void build()
        {
            ...
            if (SpellCast("SinisterStrike", ComboPointsDeficit >= 1 + Convert.ToInt32(HasAura(Auras.Broadside)))) return;
        ...
        public override void Pulse()
        {
            ...
            {
                if (WoW.CurrentEnemyUnitCount < 2 && Player(Status.IsMoving) && AutoAoECheckConfig && WoW.PlayerHasBuff("Blade Flurry")) { WoW.CastSpell("Blade Flurry"); return; }
            ...
            if ((Player(Status.TargetBoss) || !Player(Status.TargetBoss)) && Player(Status.InCombat) && Player(Status.TargetEnemy) && WoW.IsSpellInRange("SinisterStrike"))
            {
                ...
                AoECheck();
                if (PreCheck(Precheck.Stealth)) { stealth(); }
                ...
                if (Power(Unit.ComboPoints) >= ComboPointsSpend - (Convert.ToInt32(HasAura(Auras.Broadside)) + Convert.ToInt32(HasAura(Auras.Opportunity))) * (Convert.ToInt32(HasTalent(Talent.QuickDraw) && (!HasTalent(Talent.MarkedforDeath) || SpellCD(Spells.MarkedforDeath) > 1)))) finish();
        ...
        public class OutlulInterface : Form
        {
            ...
            public OutlulInterface()
            {
                System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(OutlulInterface));
...
[SpellBook.db]
Range,193315,SinisterStrike,
Spell,13750,AdrenalineRush,
Spell,8676,Ambush,
Spell,199804,BetweentheEyes,
Spell,13877,BladeFlurry,
Spell,195627,Opportunity,
Spell,185763,PistolShot,
Spell,193316,RolltheBones,
Spell,2098,Dispatch,
Spell,193315,SinisterStrike,
Spell,1784,Stealth,
Spell,1856,1856,
Spell,271877,BladeRush,
Spell,196937,GhostlyStrike,
...
Buff,193357,RuthlessPrecision
Buff,199603,SkullandCrossbones
Buff,193359,TrueBearing
Buff,5171,SliceandDice
Buff,1784,Stealth
Buff,11327,Vanish
Buff,13750,AdrenalineRush
Buff,195627,Opportunity
Buff,256171,LoadedDice

Talent,271877,BladeRush,
Talent,193531,DeeperStratagem
Talent,196937,GhostlyStrike
Talent,51690,KillingSpree
Talent,256171,LoadedDice
Talent,137619,MarkedforDeath
Talent,196938,QuickDraw
Talent,5171,SliceandDice
*/
