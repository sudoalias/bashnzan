...
namespace Pixie.Rotation
{
    public class ShadowFury : CombatRoutine
    {
        ...
        private static int SpellChargesCD(Spells spell)
        {
            switch (spell)
            {
        ...
        public void ExampleST()
        {
            ...
            //Maintain Nightblade
            if (SpellCast("Nightblade", (((!HasAura(Auras.Nightblade) && Power(Unit.ComboPoints) >= 3) || (AuraRemaining(Auras.Nightblade) <= 5 && Power(Unit.ComboPoints) >= 5))))) return;

            //Activate Shadow Blades Icon Shadow Blades if available.
            //if (SpellCast("Shadow Blades"))
            //{
            ...
            if (SpellCast("Goremaws Bite", ((HasAura(Auras.Stealth) || HasAura(Auras.ShadowDance)) && ComboPointsDeficit >= 2 && EnergyDeficit > 50))) return;
            
            if (SpellCast("Shadowstrike", (PreCheck(Precheck.Stealth) && ((!HasTalent(Talent.Anticipation) && ComboPointsDeficit >= 1) || (HasTalent(Talent.Anticipation) && Power(Unit.ComboPoints) < 7))))) return;

            if (SpellCast("Shadow Dance", (((!HasTalent(Talent.EnvelopingShadows) && (WoW.PlayerSpellCharges("ShadowDanceCharge") == 2)) || (HasTalent(Talent.EnvelopingShadows) && (WoW.PlayerSpellCharges("ShadowDanceCharge") == 3))) && Power(Unit.Energy) > 50 && (WoW.PlayerCooldownTimeRemaining("Death from Above") < 4 || WoW.PlayerCooldownTimeRemaining("Symbols of Death") < 4)))) return;
            
            if (SpellCast("Backstab", (WoW.CanCast("Shadow Dance") && WoW.PlayerCooldownTimeRemaining("Death from Above") < 1 && WoW.CanCast("Symbols of Death") && ComboPointsDeficit > 0 && EnergyDeficit > 60))) return;
            ...
            if (SpellCast("Symbols of Death", (WoW.CanCast("Shadow Dance") && WoW.CanCast("Death from Above") && EnergyDeficit > 40 && Power(Unit.ComboPoints) >= 5))) return;
            ...
            if (SpellCast("Death from Above", (HasAura(Auras.SymbolsofDeath) && WoW.CanCast("Shadow Dance") && Power(Unit.ComboPoints) >= 5))) return;

            //Activate Vanish Icon Vanish and then Shadowstrike Icon Shadowstrike when available if you are at or below 3 Combo Points.
            /*if (SpellCast("Vanish") && ComboPointsDeficit > 1 && !PreCheck(Precheck.Stealth))
            {
            ...
            if (SpellCast("Eviscerate", ((!HasTalent(Talent.Anticipation) && ComboPointsDeficit == 0) || (HasTalent(Talent.Anticipation) && Power(Unit.ComboPoints) >= 5)))) return;

            //Cast Backstab Icon Backstab to generate Combo Points(from behind the target whenever possible).
            if (SpellCast("Backstab", ComboPointsDeficit > 0)) return;
