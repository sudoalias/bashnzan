...
namespace Pixie.Rotation
{
    public class ShadowFury : CombatRoutine
    {
        ...
        public void CDRotation()
        {
            ...
            if (SpellCast("Goremaws Bite", !PreCheck(Precheck.Stealth) && SHDCDFractional <= SHDFractional && ((ComboPointsDeficit >= 4 - Convert.ToInt32(CombatSec < 10) * 2 & EnergyDeficit > 50 + Convert.ToInt32(HasTalent(Talent.Vigor)) * 25 - Convert.ToInt32(CombatSec >= 10) * 15)))) return;
...
//            if (SpellCast("Vanish", Power(Unit.Energy) >= 55 - Convert.ToInt32(HasTalent(Talent.ShadowFocus)) * 10 && PreCheck(Precheck.DSH_DFA) && (HasAura(Auras.SymbolsofDeath)) && SHDCDFractional <= SHDFractional && !HasAura(Auras.ShadowDance) && !HasAura(Auras.Stealth) && (AuraRemaining(Auras.Nightblade) >= WoW.PlayerCooldownTimeRemaining("Death from Above") + 6))
        ...
        public void StealthedRotation()
        {
            ...
            if (SpellCast("Shadowstrike", PreCheck(Precheck.Stealth))) return;
        ...
        public void StealthCDRotation()
        {
            ...
            if (SpellCast("Shadow Dance", (WoW.PlayerCooldownTimeRemaining("Death from Above") > 6 && SHDCDFractional >= SHDFractional))) return;
        ...
        public void FinishRotation()
        {
            ...
            if (SpellCast("Nightblade", (!HasTalent(Talent.DarkShadow) || !HasAura(Auras.ShadowDance)) && (Mantle_Duration == 0 || AuraRemaining(Auras.Nightblade) <= Mantle_Duration) && (AuraRemaining(Auras.Nightblade) < 3) && (Power(Unit.EnemyCount) < 4 && !PreCheck(Precheck.DSH_DFA) || !HasAura(Auras.SymbolsofDeath)))) return;
            ...
            if (SpellCast("Nightblade", (AuraRemaining(Auras.Nightblade) < SpellCD(Spells.SymbolsofDeath) + 10 && SpellCD(Spells.SymbolsofDeath) <= 5 + Convert.ToInt32(Power(Unit.ComboPoints) == 6)))) return;
            ...
            if (SpellCast("Death from Above", (!HasTalent(Talent.DarkShadow) || (!HasAura(Auras.ShadowDance) || Power(Unit.EnemyCount) >= 4) && (HasAura(Auras.SymbolsofDeath) || SpellCD(Spells.SymbolsofDeath) >= 10 + Convert.ToInt32(HasSet(Bonus.T20Four)) * 5) && AuraRemaining(Auras.TheFirstoftheDead) < 1 && (HasAura(Auras.FinalityEviscerate) || Power(Unit.EnemyCount) < 4)))) return;
        ...
        public override void Pulse()
        {
            ...
            if ((Player(Status.TargetBoss) || !Player(Status.TargetBoss)) && Player(Status.InCombat) && Player(Status.TargetEnemy) && WoW.IsSpellInRange("Backstab"))
            {
                ...
                if (Power(Unit.ComboPoints) >= 5 + 3 * Convert.ToInt32(HasAura(Auras.TheFirstoftheDead) && HasTalent(Talent.Anticipation)) + Convert.ToInt32(HasTalent(Talent.DeeperStratagem) && !HasAura(Auras.ShadowBlades) && (Mantle_Duration == 0 || HasSet(Bonus.T20Four)) && (!HasAura(Auras.TheFirstoftheDead) || PreCheck(Precheck.DSH_DFA))) || (Power(Unit.ComboPoints) >=4 && ComboPointsDeficit <= 2 && Power(Unit.EnemyCount) >= 3 && Power(Unit.EnemyCount) <= 4)) FinishRotation();
                ...
                if (PreCheck(Precheck.DSH_DFA) && SpellCD(Spells.SymbolsofDeath) <= 1 && Power(Unit.ComboPoints) >= 2 && HasItem(Item.TheFirstoftheDead) && Power(Unit.EnemyCount) < 2) FinishRotation();
