...
{
    public class FrozenFury : CombatRoutine
    {
        #region Startup
        ...
        {
            get { return "FrozenFury, Frost Death Knight rotation - version:" + _versionnumber; }
        }
        ...
        public override void Initialize()
        {
            ...
            Interface = new FrozenFuryInterface();
        ...
        private FrozenFuryInterface Interface { get; set; }
        ...
        {
            WriteLog("FrozenFury by Bash - v" + _versionnumber);
        ...
        public override void Pulse()
        {
            ...
            if ((WoW.PlayerHasTarget || WoW.TargetIsBoss) && WoW.PlayerIsInCombat && WoW.TargetIsEnemy && !WoW.PlayerIsChanneling && WoW.IsSpellInRange("Obliterate"))
            {
                ...
                if (WoW.IsTalentAvailable("BreathofSindragosa"))
                {
                    //Rotation during Breath of Sindragosa
                    if (WoW.PlayerHasPermaBuff("BreathofSindragosa"))
                    {
                        if (WoW.CanCast("EmpowerRuneWeapon"))
                        {
                            WoW.CastSpell("EmpowerRuneWeapon");
                            return;
                        }
                        //Obliterate if you have less than 30 Runic Power.
                        if (WoW.CanCast("Obliterate") && WoW.CurrentRunes >= 2 && WoW.CurrentRunicPower <= 30)
                        {
                            WoW.CastSpell("Obliterate");
                            return;
                        }
                        //Remorseless Winter if you have specced into Gathering Storm.
                        if (WoW.CanCast("RemorselessWinter") && WoW.IsTalentAvailable("GatheringStorm") && WoW.CurrentRunes >= 1)
                        {
                            WoW.CastSpell("RemorselessWinter");
                            return;
                        }
                        //Howling Blast if you have a proc of  Rime.
                        if (WoW.CanCast("HowlingBlast") && WoW.PlayerHasBuff("Rime"))
                        {
                            WoW.CastSpell("HowlingBlast");
                            return;
                        }
                        //Obliterate if you at 5 or more Runes or if you have less than 45 Runic Power.
                        if (WoW.CanCast("Obliterate") && WoW.CurrentRunes >= 2 && WoW.CurrentRunicPower <= 30)
                        {
                            WoW.CastSpell("Obliterate");
                            return;
                        }
                        //Frostscythe if you have a proc of  Killing Machine and you've specced into  Frostscythe.
                        if (WoW.CanCast("Frostscythe") && WoW.PlayerHasBuff("KillingMachine") && WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 1)
                        {
                            WoW.CastSpell("Frostscythe");
                            return;
                        }
                        //Remorseless Winter.
                        if (WoW.CanCast("RemorselessWinter") && WoW.CurrentRunes >= 1)
                        {
                            WoW.CastSpell("RemorselessWinter");
                            return;
                        }
                        //Obliterate if you are at 80 Runic Power or less.
                        if (WoW.CanCast("Frostscythe") && WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 1 && WoW.CurrentRunicPower <= 80)
                        {
                            WoW.CastSpell("Frostscythe");
                            return;
                        }
                        if (WoW.CanCast("Obliterate") && WoW.CurrentRunes >= 2 && WoW.CurrentRunicPower <= 80)
                        {
                            WoW.CastSpell("Obliterate");
                            return;
                        }
                    ...
                    if (!WoW.PlayerHasPermaBuff("BreathofSindragosa"))
                    {
                        if (EnemyCount == 1)
                        {
                            ...
                            //Frost Strike if your  Remorseless Winter is within 2 GCDs of coming off cooldown and you've specced into  Gathering Storm (We dump Runic Power so that we can spend as many runes as possible during  Remorseless Winter).
                            if (WoW.CanCast("FrostStrike") && WoW.IsTalentAvailable("GatheringStorm") && WoW.PlayerCooldownTimeRemaining("RemorselessWinter") <= (CurrentGlobalCooldown * 2) && WoW.CurrentRunicPower >= 25)
                            {
                                WoW.CastSpell("FrostStrike");
                                return;
                            }
                            ...
                            //Obliterate if you have more than 2 runes and you've specced into  Frozen Pulse).
                            if (WoW.CanCast("Obliterate") && WoW.CurrentRunes > 2 && WoW.IsTalentAvailable("FrozenPulse"))
                            {
                                WoW.CastSpell("Obliterate");
                                return;
                            }
                            //Frost Strike if you are about to cap Runic Power. (80 + Runic Power)
                            if (WoW.CanCast("FrostStrike") && ((WoW.CurrentRunicPower >= 80 && WoW.PlayerCooldownTimeRemaining("BreathofSindragosa") >= 15) || WoW.CurrentRunicPower >= 95))
                            {
                                WoW.CastSpell("FrostStrike");
                                return;
                            }
                            //Frostscythe if you have a proc of  Killing Machine and less than 4 runes ready(Obliterate if you have 4 or more runes or if you haven't specced into  Frostscythe).
                            if (WoW.CanCast("Frostscythe") && WoW.PlayerHasBuff("KillingMachine") && WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 1 && WoW.CurrentRunes < 4)
                            {
                                WoW.CastSpell("Frostscythe");
                                return;
                            }
                            if (WoW.CanCast("Obliterate") && (WoW.CurrentRunes >= 4 || (!WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 2)) && WoW.PlayerHasBuff("KillingMachine"))
                            {
                                WoW.CastSpell("Obliterate");
                                return;
                            }
                            if (WoW.CanCast("Obliterate") && WoW.CurrentRunes == 6)
                            {
                                WoW.CastSpell("Obliterate");
                                return;
                            }
                            //Obliterate
                            if (WoW.CanCast("Obliterate") && WoW.CurrentRunes >= 2 && WoW.CurrentRunicPower <= 72)
                            {
                                WoW.CastSpell("Obliterate");
                                return;
                            }
                            //Frost Strike
                            if (WoW.CanCast("FrostStrike") && WoW.CurrentRunicPower >= 25 && WoW.PlayerCooldownTimeRemaining("BreathofSindragosa") >= 15)
                            {
                                WoW.CastSpell("FrostStrike");
                                return;
                            }
                        ...
                        if (EnemyCount >= 2)
                        {
                            ...
                            {
                                WoW.CastSpell("GlacialAdvance");
                                return;
                            }
                            if (WoW.CanCast("FrostStrike") && WoW.IsTalentAvailable("IcyTalons") && WoW.PlayerBuffTimeRemaining("IcyTalons") <= CurrentGlobalCooldown && WoW.CurrentRunicPower >= 25 && (!WoW.IsTalentAvailable("GlacialAdvance") || WoW.CurrentRunicPower < 30))
                            {
                                WoW.CastSpell("FrostStrike");
                                return;
                            }
                            //Remorseless Winter if you've specced into  Gathering Storm.
                            if (WoW.CanCast("RemorselessWinter") && WoW.IsTalentAvailable("GatheringStorm") && WoW.CurrentRunes >= 1)
                            {
                                WoW.CastSpell("RemorselessWinter");
                                return;
                            }
                            //Glacial Advance if you're about to cap Runic Power (80+) and you've specced into Glacial Advance.
                            if (WoW.CanCast("GlacialAdvance") && WoW.IsTalentAvailable("GlacialAdvance") && WoW.CurrentRunicPower >= 80)
                            {
                                WoW.CastSpell("GlacialAdvance");
                                return;
                            }
                            ...
                            //Frostscythe if you have a proc of  Killing Machine and have  Frostscythe talented.
                            if (WoW.CanCast("Frostscythe") && WoW.PlayerHasBuff("KillingMachine") && WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 1)
                            {
                                WoW.CastSpell("Frostscythe");
                                return;
                            }
                            //Frost Strike if you're about to cap Runic Power (80+).
                            if (WoW.CanCast("FrostStrike") && ((WoW.CurrentRunicPower >= 80 && WoW.PlayerCooldownTimeRemaining("BreathofSindragosa") >= 15) || WoW.CurrentRunicPower >= 95))
                            {
                                WoW.CastSpell("FrostStrike");
                                return;
                            ...
                            //Frostscythe if you have Frostscythe talented.
                            if (WoW.CanCast("Frostscythe") && WoW.IsTalentAvailable("Frostscythe") && WoW.CurrentRunes >= 1 && WoW.CurrentRunicPower <= 82)
                            {
                                WoW.CastSpell("Frostscythe");
                                return;
                            }
                            ...
                            //Glacial Advance if you've specced into  Glacial Advance.
                            if (WoW.CanCast("GlacialAdvance") && WoW.IsTalentAvailable("GlacialAdvance") && WoW.CurrentRunicPower >= 30)
                            {
                                WoW.CastSpell("GlacialAdvance");
                                return;
                            }
                ...
                //Use Frost Strike Icon Frost Strike, if you have 75 + Runic Power.
        ...
        public class FrozenFuryInterface : Form
        {
            ...
            public FrozenFuryInterface()
            {
                System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FrozenFuryInterface));
...
[SpellBook.db]
Spell,196770,RemorselessWinter,D1
Spell,49020,Obliterate,D1
Spell,49184,HowlingBlast,D1
Spell,49143,FrostStrike,D1
Spell,207230,Frostscythe,D1
Spell,47568,EmpowerRuneWeapon,D1
...
Buff,59052,Rime
Buff,152279,BreathofSindragosa
Buff,51124,KillingMachine
Buff,194879,IcyTalons

Debuff,55095,FrostFever

Talent,253593,InexorableAssault
Talent,194878,IcyTalons
Talent,281208,ColdHeart
Talent,207104,RunicAttenuation
Talent,207061,MurderousEfficiency
Talent,57330,HornofWinter
Talent,276079,DeathsReach
Talent,108194,Asphyxiate
Talent,207167,BlindingSleet
Talent,207142,Avalanche
Talent,194909,FrozenPulse
Talent,207230,Frostscythe
Talent,207200,Permafrost
Talent,212552,WraithWalk4
Talent,48743,DeathPact
Talent,194912,GatheringStorm
Talent,194913,GlacialAdvance
Talent,279302,Frostwyrm'sFury
Talent,207126,Icecap
Talent,281238,Obliteration
Talent,152279,BreathofSindragosa
