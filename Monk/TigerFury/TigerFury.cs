...
namespace Pixie.Rotation
{
    public class TigerFury : CombatRoutine
    {
        ...
        {
            get { return "TigerFury, Windwalker Monk rotation - version:" + _versionnumber; }
        }
        ...
        public override void Initialize()
        {
            ...
            Interface = new TigerFuryInterface();
        ...
        private TigerFuryInterface Interface { get; set; }
        ...
        {
            WriteLog("TigerFury by Bash - v" + _versionnumber);
        ...
        {
            get { return (100 - Power(Unit.Energy)); }
        }

        private static float TimeToMaxEnergy
        {
            get { return (EnergyDeficit / EnergyRegen); }
        }
        ...
        public void CDRotation()
        {
            ...
            if (SpellCast("ArcaneTorrent", (PlayerRace(Race.BloodElf) && MaxChi - Power(Unit.Chi) >= 1 && TimeToMaxEnergy >= 0.5))) return;
            ...
            if (SpellCast("TouchofDeath", (((HasTalent(Talent.Serenity) && SpellCD(Spells.Serenity) <= 1) | Power(Unit.Chi) >= 2) && (SpellCD(Spells.StrikeoftheWindlord) < 8 || SpellCD(Spells.FistsofFury) <= 4) && SpellCD(Spells.RisingSunKick) < 7 && !WoW.PlayerWasLastCastedSpell("TouchofDeath") && !HasAura(Auras.TouchofDeath)))) return;
        }
        ...
        public void SEFRotation()
        {
            ...
            if (SpellCast("TigerPalm", (!WoW.PlayerWasLastCastedSpell("TigerPalm") && Power(Unit.Energy) == 100 && Power(Unit.Chi) < 1))) return;
            //actions.sef +=/ arcane_torrent,if= chi.max - chi >= 1 & energy.time_to_max >= 0.5
            if (SpellCast("ArcaneTorrent", (MaxChi - Power(Unit.Chi) >= 1 && TimeToMaxEnergy >= 0.5))) return;
            //actions.sef +=/ call_action_list,name = cd
            CDRotation();
            //actions.sef +=/ storm_earth_and_fire,if= !buff.storm_earth_and_fire.up
            if (SpellCast("StormEarthandFire", (!HasTalent(Talent.Serenity) && !HasAura(Auras.StormEarthandFire)))) return;
        ...
        public void STRotation()
        {
            ...
            if (SpellCast("EnergizingElixir", (HasTalent(Talent.EnergizingElixir) && Power(Unit.Chi) <= 1 && (SpellCD(Spells.RisingSunKick) == 0 || SpellCD(Spells.StrikeoftheWindlord) == 0 || Power(Unit.Energy) < 50)))) return;
            ...
            if (SpellCast("TigerPalm", (Power(Unit.Energy) >= 50 && !WoW.PlayerWasLastCastedSpell("TigerPalm") && TimeToMaxEnergy <= 0.5 && MaxChi - Power(Unit.Chi) >= 2))) return;
            //actions.st +=/ strike_of_the_windlord,if= !talent.serenity.enabled | cooldown.serenity.remains >= 10
            if (SpellCast("StrikeoftheWindlord", (Power(Unit.Chi) >= 2 && (!HasTalent(Talent.Serenity) || SpellCD(Spells.Serenity) >= 10)))) return;
            //actions.st +=/ rising_sun_kick,cycle_targets = 1,if= ((chi >= 3 & energy >= 40) | chi >= 5) & (!talent.serenity.enabled | cooldown.serenity.remains >= 6)
            if (SpellCast("RisingSunKick", (((Power(Unit.Chi) >= 3 && Power(Unit.Energy) >= 40) || Power(Unit.Chi) >= 5) && (!HasTalent(Talent.Serenity) || SpellCD(Spells.Serenity) >= 6)))) return;
            ...
            if (SpellCast("FistsofFury", (Power(Unit.Chi) >= 3 && HasTalent(Talent.Serenity) && !HasItem(Item.DrinkingHornCover) && SpellCD(Spells.Serenity) >= 5 && TimeToMaxEnergy > 2 && !Player(Status.IsChanneling)))) return;
            ...
            if (SpellCast("FistsofFury", (Power(Unit.Chi) >= 3 && HasTalent(Talent.Serenity) && HasItem(Item.DrinkingHornCover) && (SpellCD(Spells.Serenity) >= 15 || SpellCD(Spells.Serenity) <= 4) && TimeToMaxEnergy > 2))) return;
            //actions.st +=/ fists_of_fury,if= !talent.serenity.enabled & energy.time_to_max > 2
            if (SpellCast("FistsofFury", (Power(Unit.Chi) >= 3 && !HasTalent(Talent.Serenity) && TimeToMaxEnergy > 2))) return;
            //actions.st +=/ rising_sun_kick,cycle_targets = 1,if= !talent.serenity.enabled | cooldown.serenity.remains >= 5
            if (SpellCast("RisingSunKick", (Power(Unit.Chi) >= 2 && !HasTalent(Talent.Serenity) || SpellCD(Spells.Serenity) >= 5))) return;
            //actions.st +=/ whirling_dragon_punch
            if (SpellCast("WhirlingDragonPunch",  SpellCD(Spells.FistsofFury) > 0 && SpellCD(Spells.RisingSunKick) > 0 && HasTalent(Talent.WhirlingDragonPunch))) return;
            ...
            if (SpellCast("BlackoutKick", (Power(Unit.Chi) >= 1 && !WoW.PlayerWasLastCastedSpell("BlackoutKick") && MaxChi - Power(Unit.Chi) >= 1 && HasSet(Bonus.T21Four) && (!HasSet(Bonus.T19Two) || HasTalent(Talent.Serenity) || HasAura(Auras.BlackoutKick))))) return;
            ...
            if (SpellCast("SpinningCraneKick", (Power(Unit.Chi) >= 3 && (Power(Unit.EnemyCount) >= 3 || (HasAura(Auras.BlackoutKick) && MaxChi - Power(Unit.Chi) >= 0)) && !WoW.PlayerWasLastCastedSpell("SpinningCraneKick") && HasSet(Bonus.T21Four)))) return;
            ...
            if (SpellCast("CracklingJadeLightning", (HasItem(Item.TheEmperorsCapacitor) && AuraStacks(Auras.TheEmperorsCapacitor) >= 19 && TimeToMaxEnergy > 3))) return;
            ...
            if (SpellCast("CracklingJadeLightning", (HasItem(Item.TheEmperorsCapacitor) && AuraStacks(Auras.TheEmperorsCapacitor) >= 14 && SpellCD(Spells.Serenity) < 13 && HasTalent(Talent.Serenity) && TimeToMaxEnergy > 3))) return;
            //actions.st +=/ spinning_crane_kick,if= active_enemies >= 3 & !prev_gcd.1.spinning_crane_kick
            if (SpellCast("SpinningCraneKick", (Power(Unit.Chi) >= 3 && Power(Unit.EnemyCount) >= 3 && !WoW.PlayerWasLastCastedSpell("SpinningCraneKick")))) return;
            //actions.st +=/ rushing_jade_wind,if= chi.max - chi > 1 & !prev_gcd.1.rushing_jade_wind
            if (SpellCast("RushingJadeWind", (Power(Unit.Chi) >= 1 && HasTalent(Talent.RushingJadeWind) && MaxChi - Power(Unit.Chi) > 1 && !WoW.PlayerWasLastCastedSpell("RushingJadeWind")))) return;
            ...
            if (SpellCast("BlackoutKick", ((Power(Unit.Chi) >= 1 || HasAura(Auras.BlackoutKick) || (HasTalent(Talent.EnergizingElixir) && SpellCD(Spells.EnergizingElixir) < SpellCD(Spells.FistsofFury))) && ((SpellCD(Spells.RisingSunKick) > 1 && (SpellCD(Spells.StrikeoftheWindlord) > 1) || Power(Unit.Chi) > 2) && (SpellCD(Spells.FistsofFury) > 1 || Power(Unit.Chi) > 3) || WoW.PlayerWasLastCastedSpell("TigerPalm")) && !WoW.PlayerWasLastCastedSpell("BlackoutKick")))) return;
            //actions.st +=/ chi_wave,if= energy.time_to_max > 1
            if (SpellCast("ChiWave", (HasTalent(Talent.ChiWave) && TimeToMaxEnergy > 1))) return;
            //actions.st +=/ chi_burst,if= energy.time_to_max > 1
            if (SpellCast("ChiBurst", (HasTalent(Talent.ChiBurst) && TimeToMaxEnergy > 1))) return;
            //actions.st +=/ tiger_palm,cycle_targets = 1,if= !prev_gcd.1.tiger_palm & (chi.max - chi >= 2 | energy.time_to_max < 1)
            if (SpellCast("TigerPalm", (Power(Unit.Energy) >= 50 && !WoW.PlayerWasLastCastedSpell("TigerPalm") && (MaxChi - Power(Unit.Chi) >= 2 || TimeToMaxEnergy < 1)))) return;
            //actions.st +=/ chi_wave
            if (SpellCast("ChiWave", HasTalent(Talent.ChiWave))) return;
            //actions.st +=/ chi_burst
            if (SpellCast("ChiBurst", HasTalent(Talent.ChiBurst))) return;
        }
        ...
        public override void Pulse()
        {
            ...
            if ((Player(Status.TargetBoss) || !Player(Status.TargetBoss)) && Player(Status.InCombat) && Player(Status.TargetEnemy) && !Player(Status.IsChanneling) && WoW.IsSpellInRange("TigerPalm"))
            {
                ...
                if (!HasTalent(Talent.Serenity) && (HasAura(Auras.StormEarthandFire) || SpellCD(Spells.StormEarthandFire) == 0/* || SpellCharges(Spells.StormEarthandFire) == 2*/)) SEFRotation();
        ...
        public class TigerFuryInterface : Form
        {
            ...
            public TigerFuryInterface()
            {
                System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(TigerFuryInterface));
...
Spell,129597,ArcaneTorrent,D1
Spell,100780,TigerPalm,D1
Spell,137639,StormEarthandFire,D1
Spell,152173,Serenity,D1
Spell,107428,RisingSunKick,D1
Spell,205320,StrikeoftheWindlord,D1
...
Item,137097,DrinkingHornCover,D1
Item,144239,TheEmperorsCapacitor,D1
